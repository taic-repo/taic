...
#[no_mangle]
fn main() {
    println!("Start Taic multi software interrupt test ...");
    let lq0 = TAIC.alloc_lq(1, 0).unwrap();
    LQ.init_by(lq0);
    LQ.whart(axhal::cpu::this_cpu_id());
    LQ.register_receiver(1, 2, 0, 0x107);
    LQ.register_receiver(1, 2, 2, 0x109);
    ...
    // }
    lq1.send_intr(1, 0, 0);
    // while unsafe { INT_LATENCY.len() } < 1 {}
    println!("wait a moment");
    lq1.send_intr(1, 0, 2);
    // while unsafe { INT_LATENCY.len() } < 2 {}
    // println!("lq0 deq {:?}", LQ.task_dequeue());
    // println!("lq0 deq {:?}", LQ.task_dequeue());
